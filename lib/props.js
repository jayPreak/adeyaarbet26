...
// Resolves before the first QF kickoff, so only group stage + R32 + R16
// (96 of the 104 matches) count. Line scaled down from the full-tournament
// 269.5 by 96/104.
export const TOTAL_GOALS_MATCH_COUNT = 96;
export const TOTAL_GOALS_LINE = 248.5;
...
// Sum of goals scored across finished matches (score = [home, away]).
export function goalsSoFar(matches = []) {
  return matches.reduce((sum, m) => {
    if (m.status !== 'finished' || !m.score) return sum;
    const [h, a] = m.score;
    return sum + (h || 0) + (a || 0);
  }, 0);
}
