...
/**
 * Total generated ("top-up") funds for a user.
 *
 * Top-ups are modeled as `_topup` bets and are deliberately excluded from
 * settlement and P&L — they add to a wallet but never count as winnings, so a
 * player can't inflate their settlement by generating tokens they never stake.
 * This surfaces that amount purely for transparency: how much of a wallet was
 * generated and therefore won't settle.
 *
 * @param {Array<{match_id: string, payout: number|null}>} bets
 * @returns {number}
 */
export function computeGenerated(bets) {
  return bets
    .filter(b => b.match_id === '_topup')
