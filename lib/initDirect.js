...
async function _fetchInitDirect(userId) {
  // NOTE: PostgREST enforces max-rows=1000 by default. `bets` currently has
  // ~1100+ rows and grows every match. We MUST use .range(0, 9999) on any
  // query that fetches "all bets" or we silently drop rows — that dropped
  // cup_winner picks (settled/pending) and made settlement math wrong.
  const [betsRes, schedRes, poolRes, profilesRes, cupWinnerRes, challengesRes] = await Promise.all([
    ...
    supabaseBrowser.from('match_schedule').select('id, kickoff_ts').range(0, 9999),
    supabaseBrowser
      .from('bets')
      ...
      .range(0, 9999),
    supabaseBrowser.from('profiles').select('id, display_name, avatar_url').range(0, 999),
    userId
      ? supabaseBrowser
          .from('bets')
          .select('*')
    ...
    userId
      ? supabaseBrowser
          // Include ALL statuses (open/accepted/settled/void/expired/declined/cancelled)
          // so P&L graph can label historical duel bets with opponent info.
          // Downstream filter (BettingContext) narrows to open+accepted for "active duels" UI.
          .from('challenges')
