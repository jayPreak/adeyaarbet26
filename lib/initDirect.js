...
import { normalizeToZeroSum, computeSettlement, computeNetPositions } from '@/lib/settlement';
...
export async function fetchInitDirect(userId, { force = false } = {}) {
  if (!supabaseBrowser) return null;
  return dedupedFetch(`init:${userId || 'anon'}`, () => _fetchInitDirect(userId), force ? 0 : undefined);
}

async function _fetchInitDirect(userId) {
  // ONE bets query that fetches everything we need across all views:
  //   - user's bets (for account P&L, cup-winner detection)
  //   - all other users' bets excluding cancelled (for pool computation)
  // Filtering client-side is cheap and Supabase's free-tier pooler serializes
  // parallel queries anyway, so fewer round trips = faster overall.
  const allBetsPromise = supabaseBrowser
    .from('bets')
    .select('id, match_id, user_id, pick, amount, status, payout, kind, created_at, resolved_at, profiles(display_name, avatar_url)')
    .neq('match_id', '_topup')
    .order('created_at', { ascending: false });

  const [allBetsRes, schedRes, profilesRes, challengesRes] = await Promise.all([
    allBetsPromise,
  ...
  // Derive the three views we need from the one query
  const _all = allBetsRes.data || [];
  const userBets = userId ? _all.filter(b => b.user_id === userId) : [];
  const nonCancelledBets = _all.filter(b => b.status !== 'cancelled');
  const myCupWinnerBet = userId
    ? userBets.find(b => b.kind === 'cup_winner' && b.status === 'pending') || null
    : null;

  // Shim the old response shape so the rest of the function is unchanged.
  const betsRes = { data: userBets };
  const poolRes = { data: nonCancelledBets };
  const cupWinnerRes = { data: myCupWinnerBet ? [myCupWinnerBet] : [] };
