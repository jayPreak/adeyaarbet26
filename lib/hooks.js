...
export function useUser() {
  ...
  // Track the last user id we fetched a profile for. Supabase's onAuthStateChange
  // fires INITIAL_SESSION immediately after mount, which used to trigger a
  // duplicate profile fetch (~4-7s cold on top of the getSession one). Skip if
  ...
  useEffect(() => {
    ...
    if (supabaseBrowser) {
      const { data } = supabaseBrowser.auth.onAuthStateChange((event, session) => {
        ...
        // Skip if we've already fetched this user's profile (INITIAL_SESSION
        // fires right after the initial getSession() and would double-fetch).
        if (lastFetchedUserId.current === session.user.id) return;

        // On genuine auth change, re-fetch profile from DB
        const meta = session.user.user_metadata || {};
        const authUser = {
          id: session.user.id,
          username: meta.username || meta.preferred_username || session.user.email?.split('@')[0] || 'user',
